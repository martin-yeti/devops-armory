...
pub async fn filter_pod_metrics(
    google_project_id: web::Path<String>,
    query: web::Query<ListPodMetricsQuery>,
    data: web::Data<AppData>,
) -> Result<web::Json<Vec<PodMetric>>, ConfigViewError> {
    ...
    let items = {
        async_read_query::<_, _, ConfigViewError>(data.db_pool.clone(), move |connection| {
            Ok(queries::get_gcp_pod_metrics_by_project_id(
                google_project_id,
                &params.project_id,
                &params.region,
                &params.namespace,
