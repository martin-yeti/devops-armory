...
/// Total memory usage
/// Token and gke cluster endpoint, namespace and pod name need to be provided
pub async fn mem_total(
    token: String,
    ...
    loop {
        let Ok((_resp, mut connection2)) = client
                .ws(format!("https://{gke_cluster_endpoint}:443/api/v1/namespaces/{gke_cluster_namespace}/pods/{gke_pod_name}/exec?command=cat&command=/proc/meminfo&stdin=true&stdout=true&stderr=true"))
        ...
        loop {
            ...
            match response {
                Some(Ok(Frame::Binary(b))) => {
                    ...
                    let z = mem.split(" ").collect::<Vec<&str>>()[7].trim();

                    let mem_total: f64 = z.parse().expect("Parse number error");
