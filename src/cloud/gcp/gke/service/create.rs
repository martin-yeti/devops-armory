...
pub async fn create_gke_service(
    token: String,
    ...
    let create_service_request = client
        .post(format!("https://{gke_cluster_endpoint}:443/api/v1/namespaces/{gke_cluster_namespace}/services"))
        .bearer_auth(format!("{token}"))
        .timeout(Duration::from_secs(30))
        .send_json(&service_request_body)
        .await
        .expect("Request CREATE service failed");

    let mut req = create_service_request;
