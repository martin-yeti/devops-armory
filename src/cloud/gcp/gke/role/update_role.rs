...
pub async fn update_gke_role(
    ...
    let update_role_request = client
        .put(format!("https://{gke_cluster_endpoint}:443/api/v1/namespaces/{gke_cluster_namespace}/roles/{gke_role_name}"))
        .bearer_auth(format!("{token}"))
        .timeout(Duration::from_secs(30))
        .send_json(&gke_role)
        .await
        .expect("Failed to update role in current namespace");
