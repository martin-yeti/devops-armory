...
/// Create GKE Cluster Role
/// Token, GKE endpoint, namespace need to be provided
pub async fn create_gke_cluster_role(
    token: String,
    ...
    let create_role_request = client
        .post(format!("https://{gke_cluster_endpoint}:443/api/v1/namespaces/{gke_cluster_namespace}/clusterroles"))
        .bearer_auth(format!("{token}"))
        .timeout(Duration::from_secs(30))
        .send_json(&gke_role)
        .await
        .expect("Failed to create role in current namespace");

    let mut req = create_role_request;
